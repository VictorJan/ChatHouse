...
class Field:
	def __init__(self,key,required=True,**requirements):
		'''
		Goal: initialize a Field instance with a proper key, a requirement state and requirements such as : data_type,inner_data_type,regex,keys.
		...
		Exceptions:
			Raises:
				...
				ValueError :
					...
					Also if the requirements contain inner data type demand - and the data type requirement is not provided/is not iterable.
					Yet another case - if the requirements contains "keys" and the value related to latter requirement is not properly nested
		Returns: None.
		'''
		
		resolve_data_type=lambda key: str if key == 'regex' else type if re.fullmatch('^(?:inner_)?data_type$',key) else dict
		...
		assert all(map(lambda key: key in ('regex','data_type','inner_data_type','keys') and isinstance(requirements[key],resolve_data_type(key)) ,requirements)), ValueError('Allowed requirements are regex:str , data_type:type, inner_data_type and keys:dict.')

		assert hasattr(requirements.get('data_type',None),'__iter__') if requirements.get('inner_data_type',None) else True, ValueError('Having provided the inner data type, the data type shall be iterable.')

		assert (requirements.get('data_type')==dict and self.__properly_nested(copy.deepcopy(payload),True) if (payload:=requirements.get('keys')) else True), ValueError('The keys requirement is not valid.')
	...
	def validate(self,data):
		...
		conditions = lambda: map(lambda case: lambda incoming:case(incoming),(\
			lambda payload:(data_type:=contains('data_type')) and not isinstance(payload,data_type),\
			lambda payload:(inner_type:=contains('inner_data_type')) and (len(data)==0 or any(map(lambda value: not isinstance(value,inner_type),data))),\
			lambda payload:(expr:=contains('regex')) and not re.fullmatch(expr,payload),\
			lambda payload:(required_keys:=contains('keys')) and isinstance(required_keys,dict) and contains('data_type')==dict and not self.__search(payload,copy.deepcopy(required_keys),True),\
			))
