...
class LogInStrategy(Strategy):
	'''
	LogInStrategy - a class, meant to be used to perform the required actions according to the defined strategy in the accept method.
	...
	def accept(self,headers,data,**kwargs):
		'''
		Goal: validate/provide access to the login page - based on the provided tokens.
		...
		headers : meant to contain all headers data , in this particular case - Cookie field as a sign of authority for the grant_token and another Cookie field for the preaccess_token:
			...
			preaccess_token = {route:str('login'),token_type:str('preaccess')}
		Note:
		...
		Actions:
			...
			2.If the provided headers doesnt contain a valid preaccess token / or the route doesn't correspond the requested one - login:
