...
class ConnectChatStrategy(Strategy):
	...
	#Code:
		...
		#Step 0.
		#Step 1.
		if kwargs['chat_id'] is None or not kwargs['chat_id'].isnumeric() or not kwargs['authorization']['access']['valid'] or (owner:=kwargs['authorization']['access']['owner']) is None or owner.get_a_chat(int(kwargs['chat_id'])) is None:
			disconnect()
			return None
